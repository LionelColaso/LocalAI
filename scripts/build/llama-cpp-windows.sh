...
# runs as a native process -- see pkg/model/process.go (run.exe) and
# backend/cpp/llama-cpp/run-windows/.
...
cd "$ROOT"
# The launcher (run.exe) is the PE binary pkg/model/process.go starts on
# Windows; run.sh below is the stub that keeps discovery/validation/upgrade
# uniform with the other platforms. CGO_ENABLED=0 matches the existing windows
# CI builds (build-test.yaml); with a mingw gcc on PATH go would otherwise
# default to CGO on and try to link against the C toolchain.
CGO_ENABLED=0 go build -o "$STAGE/run.exe" ./backend/cpp/llama-cpp/run-windows
