...
export const SHARED_BUILD_INPUTS = [
  ...
  },
  {
    ...
    //
    // Windows: llama-cpp's run-windows launcher (backend/cpp/llama-cpp/
    // run-windows/) is stdlib-only — it links no LocalAI pkg/ code — and
    // local-ai.exe is rebuilt from the current checkout in the workflow itself,
    // so a pkg/ change never invalidates a published Windows backend image.
    // A future generic Go Windows builder would switch this to
    // isWindowsGenericGo.
    matches: file =>
