...
test("a pkg/ subtree change never rebuilds the Windows entry", () => {
  // run-windows (backend/cpp/llama-cpp/run-windows/) is stdlib-only and
  // local-ai.exe is rebuilt in the workflow itself, so the shipped image does
  // not depend on LocalAI pkg/ code.
